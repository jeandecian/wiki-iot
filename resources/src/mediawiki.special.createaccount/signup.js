...
$(function () {
  ...
  function updateForCheckbox() {
    ...
    if (checked) {
      ...
      $emailLabel.html(requiredText);
    } else {
      if ($pwds) {
        $beforePwds.after($pwds);
        $pwds = null;
      }
      $emailLabel.html(originalText);
    }
  }
    ...
    apiPromise = api
      ...
      .done(function (resp) {
        ...
        if (resp.query.users.length !== 1 || userinfo.invalid) {
          ...
          d.resolve({
            valid: false,
            messages: userinfo.cancreateerror
              ? userinfo.cancreateerror.map(function (m) {
                  return m.html;
                })
              : [],
          });
          ...
          d.resolve({
            ...
            messages: [
              mw
                .message("createacct-normalization", username, userinfo.name)
                .parseDom(),
            ],
          });
        } else {
    ...
    apiPromise = api
      ...
      .done(function (resp) {
        ...
        d.resolve({
          ...
          messages: pwinfo.validitymessages
