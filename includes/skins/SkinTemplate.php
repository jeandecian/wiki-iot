...
class SkinTemplate extends Skin
{
	...
	protected function buildPersonalUrls()
	{
		...
		if ($this->loggedin) {
			...
			if ($origTitle instanceof Title && $origTitle->isSpecialPage()) {
				list($spName, $spPar) =
					MediaWikiServices::getInstance()->getSpecialPageFactory()->resolveAlias($origTitle->getText());
	...
	public function tabAction($title, $message, $selected, $query = '', $checkEdit = false)
	{
		...
		$result = [];
		if (!$this->getHookRunner()->onSkinTemplateTabAction(
			$this,
			$title,
			$message,
			$selected,
	...
	private function getWatchLinkAttrs(
		...
		if (
			$this->getConfig()->get('WatchlistExpiry') &&
			...
			if ($userCanRead) {
				...
				if ($title->isKnown()) {
					$content_navigation['views']['view'] = $this->tabAction(
						...
						($onPage && ($action == 'view' || $action == 'purge')),
						'',
						true
					);
				...
				if ($isRemoteContent) {
					$content_navigation['views']['view-foreign'] = [
						'class' => '',
						'text' => wfMessageFallback("$skname-view-foreign", 'view-foreign')->setContext($this->getContext())->params($page->getWikiDisplayName())->text(),
				...
				if (
					$permissionManager->quickUserCan('edit', $user, $title) &&
					($title->exists() ||
				...
				if ($title->exists()) {
					...
					$content_navigation['actions']['report'] = [
						'text' => ucwords(wfMessage('report-report')->escaped()),
						'href' => SpecialPage::getTitleFor('Report', $out->getRevisionId())->getLocalURL(),
					];
					...
					if ($permissionManager->quickUserCan('deletedhistory', $user, $title)) {
						...
						if ($n) {
							...
							$msgKey = $permissionManager->quickUserCan(
								'undelete',
								$user,
								$title
				...
				if (
					$this->loggedin && $permissionManager->userHasAllRights(
						$user,
						'viewmywatchlist',
						'editmywatchlist'
					)
